...
const Dropdown = ({ list, setVisible }: DropdownProps) => (
	<ul className="divide-y divide-gray-300 divide-opacity-50">
		{list?.map(({ onClick, title, ...props }) => (
			<li
				className="cursor-pointer px-5 py-1 text-gray-500 hover:bg-gray-200 hover:text-gray-700"
				key={title}
				onClick={() => {
					onClick();
					setVisible(false);
				...
				>
					<div className="bg-white rounded">
						<div className="flex justify-end">
							<span
								className="cursor-pointer mx-2 text-gray-700"
								onClick={() => controls.setVisible(false)}
							>
								<FaTimes className="text-xs text-gray-700 sm:text-sm" />
							</span>
						</div>
						{dropList ? (
