...
export type InfoCompType = {
	...
	padding?: string;
	renderImageAs?: (props: InfoCompImageType) => JSX.Element;
	title?: string;
	titleColor?: string;
	titleColSpan?: string;
	valueColSpan?: string;
...
export const DefaultImage = ({ alt, className, src }: InfoCompImageType) => (
...
const InfoComp: FC<InfoCompType> = ({
	...
	gridStyle,
	padding,
	renderImageAs: ImageComponent = DefaultImage,
	title,
	titleColor,
	titleColSpan,
	valueColSpan,
	...
	<div className="bg-white shadow-md mt-4 mb-4 p-2 overflow-hidden sm:rounded-lg">
		{(title || description) && (
			<div className="px-4 py-5 sm:px-6">
				{title && (
					<h3
						className={`${titleColor} capitalize text-lg leading-6 font-medium`}
					>
						{title}
					</h3>
				)}
		...
		<div className="border-t border-gray-200">
			<dl>
				{infos.map((detail, index) => (
					...
					>
						<dt
							className={`${titleColSpan} text-sm font-medium text-gray-500 md:text-base`}
						>
							{detail.title}
						</dt>
						<dd
						...
						>
							{detail?.type === 'badge' ? (
								...
								<div
									className={`${
										index % 2 === 0 ? evenBorderColor : oddBorderColor
									} border-2 h-[150px] rounded-full w-[160px]`}
								>
									<ImageComponent
										alt={
										...
										src={
											typeof detail.value === 'object' ? detail.value?.src : ''
										}
									/>
								</div>
							) : (
